...
Microsoft Sentinel offers a significant amount of built-in rules, but your company has some specific use
cases that require more granular control of both rule sets and alerting. You are working on creating custom rules in Microsoft Sentinel to meet these use cases.
You need to create a rule that runs every hour and stops running if there is a single alert.

Which two options should you select in the Analytics rule wizard to meet these requirements? 
