...
Your company plans to expand its Azure footprint by deploying 10 virtual machines that will host a critical e-commerce application. Recently the company was targeted by a layer 7 DDoS attack that flooded the e- commerce app with invalid requests, causing an outage.
You need to recommend a solution to protect against these DDoS attacks.
...
You should recommend your company to use Azure Application Gateway. A DDoS attack occurs when a malicious actor tries to overload a system by sending invalid requests. Azure Application Gateway, which is a Layer 7 load balancing and traffic management service, can be used to filter these requests utilizing features offered by Azure Web Application Firewall (WAF). The gateway operates at layer 7 of the Open Systems Interconnection (OSI) model, which means it can make load balancing decisions based on complex rules, including Uniform Resource Locator (URL) based routing. This contrasts with traditional load balancers that work at layer 3 or layer 4 of the OSI model and block traffic primary based on IP addresses and ports. In this scenario, you should deploy Application Gateway WAF v2 SKU, which includes the WAF DDoS feature.

You should not recommend your company to use Azure DDoS Protection. This service works at layers 3 and 4 of the OSI model. It does not have access to layer 7 data and cannot block layer 7 DDoS attacks.
You should not recommend your company to use Azure Load Balancer. Similar to Azure DDoS Protection, Azure Load Balancer works at layer 4 of the OSI model. While this service can be used to balance network traffic loads based on Transmission Control Protocol (TCP) port numbers, it cannot be used to block layer 7 DDoS attacks.
You should not recommend your company to use Azure Traffic Manager. This service uses Domain Name System (DNS) requests to load balance traffic across your Azure footprint. When a client requests a specific host, Traffic Manager can route the request to and endpoint using rules you define. Traffic Manager is not used to block layer 7 DDoS attacks.

---

### References

[Tutorial: Protect your application gateway with Azure DDoS Network Protection](https://learn.microsoft.com/en-us/azure/application-gateway/tutorial-protect-application-gateway-ddos)  
...
Your company has deployed multiple cloud-based websites and apps using Azure services. You have been asked to recommend methods that can be used to optimize network performance and you have elected to deploy several Azure load-balancing solutions. Specifically, you have implemented the following configuration:

- An e-commerce website has been deployed using Azure Application Gateway.
- A business intelligence app has been deployed using Azure Front Door.
...
If an incoming request to the e-commerce website includes "/photos" in the URL, you can route to a pool that has been optimized for storing images.
Yes

If requests are sent to the website or the app using resolvable hostnames, you can use DNS to route traffic to servers based on rules you define.
No

If requests for the business intelligence app come from users in remote parts of the world, you can service those requests using a content delivery network (CDN).
Yes
...
If an incoming request to the e-commerce website includes "/photos" in the URL, you can route to a pool that has been optimized for storing images. Azure Application Gateway is a Layer 7 load balancing and traffic management cloud-based service from Microsoft. This means that Azure Application Gateway can make load balancing decisions based on complex rules, including Uniform Resource Locator (URL) based routing. This feature allows you to route service requests to endpoints that are optimized for a specific type of data or application capability.
If requests are sent to the website or the app using resolvable hostnames, you will not be able to use DNS to route traffic to servers based on rules you define. This requirement can be met using Azure Traffic Manager. This tool uses Domain Name System (DNS) requests to load balance traffic across your Azure footprint. When a client requests a specific host, Traffic Manager can route the request to an endpoint using rules you define.
If requests for the business intelligence app come from users in remote parts of the world, you can service those requests using a content delivery network (CDN). This is a feature of Azure Front Door, which is another Azure-based load-balancing solution. Front Door places copies of your app content in strategic locations across the world. When Front Door receives a request, it will attempt to fulfill the request from a location near where the request originated. This approach enhances performance and can increase the app's availability.
...
Your client plans to migrate on-premises databases and other services to the Azure cloud. To enhance data security, you have been hired to design a solution that will allow the data to be copied from the on-premises servers to Azure endpoints.
Your solution must meet the following requirements:

- Data must not be sent across the public internet.
- The solution should offer a minimum of 50 Gbps bandwidth.
- Latency should be reduced as much as possible.

You need to recommend which tasks to include in your solution.

Which two tasks should you recommend? Each correct answer presents part of the solution.

Choose the correct answers.

- Deploy a P2S VPN gateway.
...
You should recommend your client to implement Azure Express Route and configure a Private Link. Azure Express Route can be used to extend an on-premises network directly to the Microsoft cloud. Data sent using Express Route does not cross the public internet and is sent using a private connection. This approach increases security and reduces latency. You can connect to Express Route using one of several methods, including a point-to-point Ethernet connection. Private Link provides secure connectivity between an Azure virtual network and Microsoft services and enhances security by ensuring your data never leaves Microsoft- owned network infrastructure. Private Link can be used to connect on-premises networks to the Microsoft cloud.
You should not recommend your client to deploy a Point-to-Site (P2S) Virtual Private Network (VPN) gateway connection. P2S VPN connections are designed to connect a single endpoint to an Azure virtual network.
You should not recommend your client to create a VNet service endpoint. You can use a service endpoint to access resources in Azure from within your Azure VNet. Service endpoints are not used to connect on- premises networks to the Microsoft cloud.
You should not recommend your client to create a private endpoint. You may create a private endpoint as part of your Express Route configuration. However, this alone will not fulfill the requirements stated in the scenario.
...
You are requested to design a network solution for a multi-tier application in Azure that consists of:

- Two internet-facing web application servers as a front end

- Two App Service application servers that work as a middle tier and host videos and images in two respective directories: /videos/* and /images/*. Middle tier requests have to be routed based on different content types

- A SQL database in the back end

For web applications facing the internet you include Azure Front Door as a load balancing solution. App service applications are installed in a single Azure region only.

You need to recommend a solution to load balance the traffic for the middle tier.
...
You should include Azure Application Gateway in your solution. Azure Application Gateway is a load balancing solution operating on layer 7 of the Open Systems Interconnection (OSI) reference model. Azure Application Gateway makes routing decisions based on attributes of an HTTP request; for example, URI path or host headers. In this scenario, middle tier requests have to be routed based on different content types, so an application gateway is the best choice for the two App Service applications working as a middle tier.
You should not include Azure Front Door in your solution. Azure Front Door is an entry point that is present in different Azure regions around the world and uses the Microsoft global edge network to route traffic to your web application in any region. Azure Front Door is the best choice for internet-facing web applications. Although Azure Front Door can load balance between service endpoints across Azure regions, Application Gateway allows you to load balance within one Azure region only. Although Azure Front Door may technically work, this scenario requires no interregional load balancing for the middle tier.
You should not include Azure Load Balancer in your solution. Azure Load Balancer is a load balancing solution operating on layer 4 of the OSI reference model. With OSI layer 4, the routing decision is made based on an IP address and not on different content types, as requested in this scenario.
You should not include Azure Traffic Manager in your solution. Azure Traffic Manager provides a load balancing solution to route your user requests to multiple service points, based on DNS query and DNS response. Unlike other load balancing solutions in Azure, Traffic Manager operates on layer 7 of the OSI reference model and can load balance traffic among geographical regions. Similar to Azure Front Door, Azure Traffic Manager is the best choice for internet-facing web applications that are distributed between Azure regions.
...
CompanyA runs some of the HR workflows using an Azure logic app named LApp1. LApp1 needs access to employees' data stored in a SQL database named DB1. DB1 is still hosted in an on-premises data center (DC). Company policy prohibits DB1 from accessing the internet.
You need to design a solution to enable LApp1 to query data from DB1.
Which two resources should you include in your design? Each correct answer presents part of the solution.
...
You should include an on-premises data gateway and an Azure connection gateway resource in your
design. An on-premises data gateway provides a solution to enable Azure cloud resources to connect
securely to on-premises data sources, like SQL databases, independently of whether the resource has
internet access. Azure cloud resources, which are able to use on-premises data gateways, include Power BI,
Power Apps, Power Automate, Azure Analysis Services, and Azure Logic Apps. On-premises data gateways
are an agent installed on-premises and represent a connection broker between cloud and on-premises
resources.
You should also include an Azure connection gateway resource and connect it to your on-premises installation.
You should not include Azure application registration in your design. Application registration in Azure is a process to integrate your application in your Azure tenant. Registering an application in the Azure portal creates an application object and a service principal. An application object provides a way to set up a look and feel for the application, define a URL and an authentication method. A service principal provides a way to assign Microsoft Entra permissions for the Azure application to access Azure services. Registered Azure applications cannot act as a broker between an Azure logic app and an on-premises SQL server.
You should not include Microsoft Entra Application Proxy in your design. Microsoft Entra Application Proxy provides a method to act as a broker to access an on-premises web application for users outside of the local network. However, this does not enable access to data sources like a SQL database.
...
[Connect to on-premises data sources from Azure Logic Apps](https://learn.microsoft.com/en-us/azure/logic-apps/connect-on-premises-data-sources)  
...
You are an administrator of an engineering company, which recently migrated its on-premises web application WebApp1 to Azure. After migration, you receive complaints that WebApp1 is behaving unpredictably: at peak times it is unresponsive and at all other times its resources are not being utilized.
You need to recommend a solution to identify the locus of instability issue for WebApp1.
Which Azure Insights solution should you recommend as a starting point for the investigation?
...
You should recommend Application Insights as an investigation starting point. Azure Application Insights is a performance management and application runtime debugging tool. It can help to detect web application anomalies as well as issues and to continuously improve a web application's performance and usability. Besides possible instability in the application's code, modern web applications involve a plethora of network devices and connected services, which can cause performance and instability issues. The first step of the investigation is to pinpoint the service or network device causing the issue. Application Insights is designed for this purpose.
You should not recommend Network Insights as an investigation starting point. Network Insights is designed to get a view of the deployed network resources. It can help to trace network performance and connectivity issues. In this scenario, Network Insights could be used as the next step of the investigation. However, the investigation's starting point should be Application Insights.
You should not recommend Storage insights as an investigation starting point. Storage insights is designed to get a view of the deployed storage accounts, get information about storage performance, capacity, and availability. In this scenario, Storage insights could be one of the next steps in the investigation chain. However, the investigation's starting point should be Application Insights.
You should not recommend VM insights as an investigation starting point. VM insights is one of the Azure Insights tools to measure performance and health of your virtual machines and virtual machine scale sets. VM insights could be the following diagnostic step in resolving the issue. However, the investigation's starting point should be Application Insights.

---

### References

[Design for Azure Workbooks and Azure Insights](https://learn.microsoft.com/en-us/training/modules/design-solution-to-log-monitor-azure-resources/4-design-for-azure-workbooks-insights)  

[Deep diagnostics for web apps and services with Application Insights](https://learn.microsoft.com/en-us/previous-versions/azure/azure-monitor/app/old-overview)  
...
Your company is developing a solution based on a microservices architecture. Rather than exposing services directly to users, they should be accessed through an API gateway that supports:
Load balanced access to services
• Support for HTTP/HTTPS and layer 7 routing
SSL termination
• Autoscaling based on traffic patterns
Protection from common exploits and vulnerabilities
You need to recommend an API gateway solution that meets these requirements.
...
You should recommend Azure Application Gateway as your API gateway solution. Azure Application Gateway meets all the solution requirements. Protection against common exploits and vulnerabilities is provided through Web Application Firewall, which is a feature of Azure Application Gateway. Application Gateway is a management solution designed to manage traffic to web applications.
You should not recommend APIM. APIM is an API management suite that lets you create consistent and modern API gateways for existing back-end services. It gives you a way to publish APIs to external, partner, and internal developers. APIM supports features for managing a public-facing API, but it does not meet all the requirements such as load balancing.
You should not recommend Azure Traffic Manager. Azure Traffic Manager operates as a layer 7 DNS router and provides global load balancing across multiple regions. It is not appropriate for this scenario because it does not support SSL termination.
You should not recommend Azure Load Balancer. Load Balancer operates at layer 4 rather than layer 7 and, other than load balancing, it does not meet the scenario requirements. It does not include any security protections.
...
[Using API gateways in microservices](https://learn.microsoft.com/en-us/azure/architecture/microservices/design/gateway) 
...
[What is Azure Load Balancer?](https://learn.microsoft.com/en-us/azure/load-balancer/load-balancer-overview)  
[What is Azure Application Gateway?](https://learn.microsoft.com/en-us/azure/
application-gateway/overview)  
[Frequently asked questions about Application Gateway](https://learn.microsoft.com/en-us/azure/application-gateway/application-gateway-faq)  
...
Your company has an on-premises Active Directory (AD) domain and a Microsoft Entra subscription. You are designing an Azure internet-facing web application to be deployed in the West Europe Azure region. Network traffic for the application must support load-balanced distribution throughout the region.
Your load balancing solution must:
Provide application layer (OSI layer 7) load balancing for HTTPS protocol traffic.
Support URL-routing.
Control routing through round-robin sequencing.
Provide Secure Sockets Layer (SSL/TLS) termination.
You need to recommend a load-balancing solution. You want to minimize the solution cost and management overhead.
...
You should recommend Azure Application Gateway as your load-balancing solution. Application Gateway implements an OSI layer 7 load-balancing solution, which is necessary to balance HTTPS traffic. Load balancing is supported in a round-robin manner, cycling through available instances. Application Gateway supports URL routing. It supports SSL/TLS termination, also known as SSL offload. Therefore, traffic is unencrypted at the gateway and sent to backend servers as clear text.
You should not recommend Azure Load Balancer. You can use an Azure load balancer as a public or internal
load balancer, but it does not support all of the features needed. For example, Azure Load Balancer operates as OSI layer 4 and does not support SSL/TLS termination.
You should not recommend Azure Front Door or Azure Traffic Manager. These are both global load balancing solutions, which means they are designed for balancing traffic between regions, including on- premises instances. You do not need a global load-balancing solution because your application is deployed in a single Azure region. Also, Azure Traffic Manager does not provide support for SSL/TLS termination.
Apart from being a global balancing solution, Azure Front Door would meet the solution requirements, but it is not a recommended solution in this type of scenario, and it would be a more expensive solution to implement.
...
[What is Traffic Manager?](https://learn.microsoft.com/en-us/azure/traffic-manager/traffic-manager-overview)  
[What is Azure Front Door?](https://learn.microsoft.com/en-us/azure/frontdoor/front-door-overview)  
[Create an application gateway with an internal load balancer (ILB)](https://learn.microsoft.com/en-us/azure/application-gateway/application-gateway-ilb-arm)  
[Azure Application Gateway features](https://learn.microsoft.com/en-us/azure/application-gateway/features)  
...
Your company developed a web app named App1 and initially hosted the app on premises. Your company deploys to virtual machines (VMs) in the North Central US, South Central US, West Central US, and West US Azure regions.
You need to configure a solution that has users connect with the nearest geographically available instance of App1.
...
You should configure Azure Traffic Manager or Azure Front Door. You can configure the rules engine in Azure Front Door or configure endpoint monitoring in Azure Traffic Manager. Either solution ensures that users are connected with the nearest instance of App1.
Azure supports four load-balancing options:
Azure Traffic Manager
Azure Front Door
. Azure Load Balancer
Azure Application Gateway
Of these options, only Azure Traffic Manager and Azure Front Door provide global load balancing that can distribute traffic across different regions and can include hybrid on-premises locations.
You should not configure Azure Load Balancer or Azure Application Gateway. These are both regional load- balancing services that are used to distribute a load within instances in a region. Load balancing is supported in Azure Application Gateway through the internal load balancer (ILB) feature.
...
[What is Azure Front Door?](https://learn.microsoft.com/en-us/azure/frontdoor/front-door-overview)  
[What is Rules Engine for Azure Front Door?](https://learn.microsoft.com/en-us/azure/frontdoor/front-door-rules-engine?pivots=front-door-standard-premium)  
[What is Traffic Manager?](https://learn.microsoft.com/en-us/azure/traffic-manager/traffic-manager-overview)  
...
Your company has two Azure subscriptions named Subscription1 and Subscription2. Both subscriptions are associated with the same Active Directory (AD) tenant.

Subscription1 includes the following virtual network:

- Name: VNET1
- Address space: 10.1.0.0/16
- Location: (US) Central US
- Number of connected virtual machines (VMs): 200

Subscription2 includes the following virtual network:

- Name: VNET2
- Address space: 192.168.0.0/16
- Location: (Europe) North Europe
- Number of connected virtual machines (VMs): 100

You need to recommend a connectivity solution that enables the VMs on VNET1 and VNET2 to communicate over the Microsoft backbone infrastructure.
...
You should recommend virtual network peering. Virtual network peering enables you to connect Azure virtual networks across regions and Azure subscriptions seamlessly. For connectivity purposes, peered virtual networks appear as one network. The traffic between VMs in the peered virtual networks is routed through the Microsoft backbone infrastructure, much like traffic is routed between VMs in the same virtual network. You can peer virtual networks that exist in two different subscriptions as long as a privileged user of both subscriptions authorizes the peering and the subscriptions are associated with the same AD tenant.
You should not recommend ExpressRoute. Express Route establishes connectivity between an on-premises
network and an Azure virtual network over a private connection via a connectivity provider, not over the Microsoft backbone infrastructure.
You should not recommend Site-to-Site VPN. Site-to-Site VPN establishes connectivity over a public network, and not over Microsoft backbone infrastructure.
You should not recommend Point-to-Site VPN. Point-to-Site VPN can be used to establish connectivity between a single computer and a virtual network. Point-to-Site VPN cannot be used to establish connectivity between virtual networks. Point-to-Site VPN establishes connectivity over a public network, not over Microsoft backbone infrastructure.

---

### References

[Create, change, or delete a virtual network peering](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-manage-peering?tabs=peering-portal#requirements-and-constraintshttps://docs.microsoft.com/en-us/azure/virtual-network/virtual-network-manage-peering)  
...
A company is modernizing its infrastructure by moving workloads to the cloud. These workloads comprise virtual machines, databases, and applications.
...
[Cloud migration in the Cloud Adoption Framework](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/migrate/)  
...
Which resource should you deploy for each component? To answer, select the appropriate resources from the drop-down menus.
...
You should not use Azure Traffic Manager as the solution for either requirement. Azure Traffic Manager distributes traffic and directs users to the most appropriate service endpoint; for example, when supporting multiple instances to a web app. This can help response latency; however, it does not meet the specific requirements.
...
You should not recommend Azure Service Bus. Azure Service Bus is used for message delivery instead of events, such as traditional transaction-processing applications, where the message producer requires a guarantee that the message will be handled by a consumer.
