...
Your company is planning to migrate workloads to Azure from your data center. You are in the process of designing the perimeter security for the Azure landing zone and decide to implement a web application firewall (WAF) with Azure Front Door to protect web applications that will be hosted in the cloud.

The WAF deployment must meet the following requirements:

- WAF must be able to natively integrate with Azure Front Door features
- WAF must be able to complete specific actions if a WAF rule is triggered
- WAF must monitor all traffic and log all matched requests

You need to determine which tier and deployment mode you should configure on the WAF to meet these requirements.
For each of the following statements, select Yes if the statement is true. Otherwise, select No.

- You should use WAF with Azure Front Door Premium in prevention mode.
Yes

- You should use WAF with Azure Front Door Standard in detection mode.
No

- Custom rules are supported on a WAF with Azure Front Door Premium only.
No
...
You have the following requirements for the Web Application Firewall (WAF) deployment:
• WAF must be able to natively integrate with Azure Front Door features
• WAF must be able to complete specific actions if WAF rule is triggered
• WAF must monitor all traffic and log all matched requests
You should use WAF with Azure Front Door Premium in prevention mode. WAF with Azure Front Door Premium in prevention mode will meet the requirements. There are two WAF policy modes that you can run: detection mode and prevention mode. Detection mode is for monitoring and logging only, which does not take any actions at all. With prevention mode, it allows the WAF to not only monitor and log any matched actions, but also complete specific actions if a WAF rule match is made. For this scenario you need Azure Front Door Premium in prevention mode.
You should not use WAF with Azure Front Door Standard in detection mode. Azure Front Door Standard
with detection mode would be able to fulfill the requirement for the WAF to monitor all traffic and log all matched requests but no other requirement.
Custom rules are not only supported on a WAF with Azure Front Door Premium, as this is a feature of the Standard tier as well.
Azure Front Door Standard with prevention mode would be able to fulfill the requirement for the WAF to monitor all traffic and log all matched requests but no other requirement as Standard tier does not natively integrate with Azure Front Door features. Azure Front Door Premium with detection mode would meet meet the requirements to natively integrate with Azure Front Door features and monitor all traffic and log all matched requests but it would not be able to complete specific actions if WAF rule is triggered.
...
You need to block access to the web app for traffic originating from Australia, due to data sovereignty requirements.
...
- Configure an access restriction rule within the web app's network settings.
- Create an HTTPS redirect rule using Rules Engine Configuration.
- Create and configure a custom rule in the WAF policy associated with Azure Front Door.
...
You should create and configure a custom rule in the Web Application Firewall (WAF) policy associated with
Azure Front Door. WAF is an Azure web protection service that provides protection for web-based apps
against vulnerabilities and exploits. WAF can be used in conjunction with a number of Azure services,
including Application Gateway and Azure Front Door. Administrators can block incoming traffic to Azure
Front Door using custom rules, based on conditions such as geolocation and IP address. WAF Custom
rules provide Azure administrators with granularity and additional control, which can enhance protection to
meet various business requirements. For instance, you may have data sovereignty requirements that dictate
that your web app should not be available in certain geographical locations, and this could be a result of
legal or political issues.
You should not configure an access restriction rule within the web app's network settings. Access restriction rules provide an additional layer of security for Azure App Service workloads. It allows Azure administrators to create a list of allow or deny rules based on IP address or Azure virtual network (VNet) subnets. Priorities can also be assigned to each rule. This option is not applicable in this case, as geographic locations cannot be blocked using access restriction rules.

You should not create an App Service Environment (ASE). An ASE allows you to fully isolate an Azure App Service and integrate it with an Azure VNet. This offers full inbound and outbound connectivity control to and from your web app. This is useful when there are strict business and technical security requirements for a web app that needs to integrate with your VNets. For instance, there may be an on-premises application that needs inbound and outbound communication with the Azure web app without exposing it to the internet.
You should not create an HTTPS redirect rule using Rules Engine Configuration. In Azure Front Door, Rules Engine can be used to configure a number of rules that get triggered after the WAF but before the Azure Front Door routes. One of the rules that can be created is HTTP to HTTPS redirection rules. This is useful if you need to force all connectivity to be secure. This does not, however, address the requirements in the scenario, as it is being asked to block traffic originating from Australia.

---

### References

[What is Azure Web Application Firewall?](https://learn.microsoft.com/en-gb/azure/web-application-firewall/overview)  

[Set up a geo-filtering WAF policy for Azure Front Door](https://learn.microsoft.com/en-us/azure/web-application-firewall/afds/waf-front-door-tutorial-geo-filtering)   

[Custom rules for Web Application Firewall with Azure Front Door](https://learn.microsoft.com/en-us/azure/web-application-firewall/afds/waf-front-door-custom-rules)  
...
[Introduction to App Service Environment v2](https://learn.microsoft.com/en-us/azure/app-service/environment/intro)  

[Rules match conditions](https://learn.microsoft.com/en-us/azure/frontdoor/rules-match-conditions?tabs=portal&pivots=front-door-standard-premium)
...
![image info](./Q129_1.PNG)

You have deployed Azure Front Door in front of your web app and associated a Web Application Firewall (WAF) policy.

Your organization has the following branch locations:

Branch Office Location | IP Address
Chicago     : 195.211.42.177
New York    : 197.200.12.168
Washington  : 194.229.17.142

You have created a custom WAF policy. Please refer to the exhibit for the current configuration.
For each of the following statements, select Yes if the statement is true. Otherwise, select No.

Employees based in the New York office can access the web app.
Yes

Employees based in the Washington office can access the web app.
No

Employees based in the Chicago office can access the web app.
Yes
...
Employees based in the New York office can access the web app. The New York branch IP address has been added as an address range in the custom rule. As shown in the exhibit, the Does Not Contain value has been selected for the operation mode, and the action has been set to Deny. This means that employees from New York will be allowed to bypass the rule.
Employees based in the Washington office cannot access the web app. The IP address for Washington has not been explicitly defined in the custom rule.
Employees based in the Chicago office can access the web app. The Chicago branch IP address has been added as an address range in the custom rule. As shown in the exhibit, the Does Not Contain value has been selected for the operation mode, and the action has been set to Deny. This means that employees from Chicago will be allowed to bypass the rule.

---

### References

[Configure an IP restriction rule with a WAF for Azure Front Door](https://learn.microsoft.com/en-us/azure/web-application-firewall/afds/waf-front-door-configure-ip-restriction?tabs=browser)  

---

## Q0128:

You plan to configure DNS alias records to access Azure resources.
Which three types of DNS records can you use to configure an alias record? Each correct answer presents a complete solution.
...
DNS is used to assign human-readable names to different resources, which can then be mapped to an IP address identifier. An Address A record is used to map the IP address to a domain name. An AAAA record is used to map an IPv6 IP address to a domain name. The Canonical Name or CNAME record is used to map different aliases to a parent domain, for example mapping www.xyz.com to xyz.com. So, if you type www.xyz.com, the DNS will resolve it to xyz.com and eventually the domain IP address.
...
[Tutorial: Configure an alias record to refer to an Azure public IP address](https://learn.microsoft.com/en-us/azure/dns/tutorial-alias-pip)  
...
You are a solutions architect for a managed service provider that has clients that host their services in Azure. The company has recently onboarded a new client that hosts web services, applications, and databases on VMs in Azure with no perimeter security and with user access to the VMs via Remote Desktop Services (RDP).
As part of the onboarding review you have recommended implementing some perimeter security in a firewall to protect externally-facing services. The client has specified the following requirements shown in the table below:
Requirement
Requirement Description
High availability
The firewall must be highly available and outages to the service must be minimized.
DNS Proxy
The firewall must be able to transmit selected DNS queries via a tunnel that prevents malicious actors gaining information about the private network.
The firewall must be able to integrate with the client's existing Security Information
and Event Management (SIEM) system
SIEM Integration
Performance
The firewall must be able to scale up to 30 Gbps-bandwidth as traffic grows.
You need to recommend the most appropriate solution for these requirements. Your solution must minimize cost and use native services where possible.

What type of firewall should you recommend to the client?

Choose the correct answer

- Azure Firewall with Standard SKU
...
You should recommend Azure Firewall with the Standard SKU for the perimeter security, as this meets the client's requirements, including being a native service to Azure. There are three different SKUs for the Azure Firewall: Basic, Standard, and Premium. The Premium SKU includes everything in the Basic and Standard SKUs but with even more advanced features.
The Basic SKU only allows up to 20 Mbps-bandwidth for traffic and it does not allow DNS proxy, and therefore it is not the correct solution. The Premium and Standard SKUs can meet all the requirements; however, the Premium SKU is more costly than the Standard SKU and so does not minimize cost.
Network Virtual Appliances (NVAs) are virtual machines that can be deployed with a supported third-party firewall operating system (OS), including Cisco and Barracuda. This is not technically native to Azure and costs much more than Azure Firewall and so would not be an appropriate solution.
...
[Network Appliances](https://azure.microsoft.com/en-gb/solutions/network-appliances)  
...
Your company wants to host a new application on Azure. Users from the London and Manchester offices will use the new application. The new application has the following requirements:

- VMs must be hosted in a /24 virtual subnet

- The London and Manchester offices require a Site-to-Site VPN connection into Azure

- The application requires Azure Kubernetes Services (AKS)

- The application requires a Fortinet Firewall Virtual Appliance to facilitate perimeter security

You need to plan and design the virtual network in Azure.

Which Azure service requires a dedicated subnet?
...
Certain Azure services require a dedicated subnet to allow them to communicate with other cloud services and resources privately via a private IP address.
A VPN Gateway allows you to connect your virtual networks in Azure to your on-premises locations so users can access cloud services from on-premises. A VPN Gateway needs its own dedicated subnet called a 'Gateway Subnet' which has a /27 subnet where no other resources are hosted.
Azure can host either Windows or Linux virtual machines (VMs) that need to be hosted on a virtual network with a subnet. VMs do not require a dedicated subnet and can reside on a subnet that hosts other services like Azure Kubernetes Services (AKS) and VM scale sets.
Azure Kubernetes Services (AKS) is a serverless container service that can be hosted on Azure. It requires a network to allow users to connect to it, however it can be hosted on a share subnet and does not need a dedicated subnet like a VPN Gateway does.
Network Virtual Appliance (NVA) is a compute resource that is generally used by third party vendors to host their products on Azure. An example of this is deploying a Fortinet Firewall on Azure that is deployed as an NVA. As this is a resource used for perimeter security, it is best practice to host it in a Hub network. It does not need a dedicated subnet and can be hosted on a subnet which is used by other services as well.
---
...
During a customer engagement, you deployed and configured a single Azure Firewall instance with forced tunneling disabled. You used a hub and spoke topology for the design.
The security team reviewed the solution but would now like to introduce a Network Virtual Appliance (NVA) in Azure for additional filtering capabilities.
You need to ensure that any internet-bound connectivity passes through the NVA before reaching the internet.
What should you do?

Choose the correct answer

- Deploy an NVA and create a user-defined route on the existing Azure Firewall to force traffic from Azure Firewall to the NVA.

- Migrate to Azure Firewall Premium.

- Remove the existing Azure Firewall and create a new instance with forced tunneling enabled.

- Enable Threat intelligence in the Alert and deny mode.
...
You should remove the existing Azure Firewall and create a new instance with Forced tunneling enabled. Forced tunneling is the key feature that is required. This feature will allow you to divert traffic from Azure Firewall to the NVA. A dedicated subnet called 'AzureFirewall Management' is required for this to work. Once Azure Firewall has been configured with forced tunneling, then a user-defined route (UDR) can be associated with the 'AzureFirewallSubnet' to route internet bound traffic via an NVA first. In addition to this, forced tunneling cannot be enabled on an existing Azure Firewall. It can only be enabled at the time of creation. Therefore, the removal of the existing Azure Firewall is key.
You should not enable Azure Firewall Premium. While this will provide the customer with deeper security controls such as Intrusion Detection and Prevention System (IDPS) and Transport Layer Security (TLS) inspection, it does not address the key requirement of forcing traffic to traverse via the NVA. IDPS allows users to stay one step ahead of malicious attacks. This is done by monitoring network activity, logging the information and also blocking any malicious attempts to gain access to a network. TLS ensures that outbound network traffic is decrypted, processed, and then finally encrypted again before it is pushed to the destination.
You should not enable Threat intelligence in the Alert and deny mode. If you enable this setting in Azure Firewall, it will alert and deny traffic from/to malicious sources. The sources are established by Microsoft. This adds a layer of security, but it does not address routing from Azure Firewall to the NVA.
You should not deploy an NVA and create a user-defined route on the existing Azure Firewall. This solution would only work if force tunneling was pre-enabled on the firewall. It is not possible to enable force tunneling on an existing deployment.
...
You have deployed various applications using Azure Virtual Machines (VMs) in your company's Azure environment. You are seeing some suspicious activities happening on the VMs. You already use a network security group (NSG) to restrict access to some of the VMs.
To identify suspicious network traffic, you have decided to enable NSG flow logs to gather more information.
...
Register the Microsoft.SecurityInsights provider.
...
To enable NSG flow logs, you should enable Network Watcher in the region where your VMs are deployed. Network Watcher provides tools to monitor and collect metrics and logs from your Azure resources deployed inside a virtual network (VNet). Currently, Network Watcher only supports Infrastructure as a Service (laas) and is not intended to be used with Platform as a Service (PaaS) resources.
...
To store the flow logs, you should create a storage account. This storage account should be in the same
region and all the logs collected from NSG will be shipped to the storage account. The logs are stored in JSON format and can be viewed by downloading the files locally or shipping the files to a Security Information and Event Management system (SIEM) or an Intrusion Detection System (IDS).
...
```
https://{storageAccountName}.blob.core.windows.net/insights-logs- networksecuritygroupflowevent/resourceId=/SUBSCRIPTIONS/(subscriptionID)/RESOURCEGROUP {year}/m={month}/d={day)}/h={hour}/m=00/macAddress={macAddress)/PT1H.json
```
...
You should not register the Microsoft.SecurityInsights provider. This provider is used by Microsoft Sentinel, which is a cloud-based SIEM managed by Azure.
...
Your company currently hosts resources in a private data center. You are currently designing an Azure Landing Zone in order to migrate services into the cloud and you want to document the number of public IP addresses you will need in your environment.
The Azure Landing Zone will host the following resource types: VMs, Web Server, Bastion Host, Network Security Group (NSG), Private Load Balancer, Public Load Balancer, VPN Gateway, Azure Key Vault, and Storage Account.
You need to document the number of public IP addresses you will require within the Azure Landing Zone. Your solution must follow Microsoft best practice for deploying public IP addresses.
How many public IP addresses do you need?
...
You need a total of three public IP addresses within the Azure Landing Zone for the services you plan on hosting. The following three services will require a public IP address:

- Public Load Balancer
- VPN Gateway
- Bastion Host

Although you can assign a public IP address to VMs, including web servers, it is best practice to only allow internal access to VMs and put another public facing service in front of the VMs to add an additional layer of protection. You can use Public Load Balancers to facilitate this.
Bastion Hosts require a public IP address as they facilitate connecting to VMs without allowing direct access to the VM via Remote Desktop Protocol (RDP) or Secure Shell (SSH).
VPN Gateway is used to connect your Azure Network securely with your on-premises network, and therefore requires a public IP address to make that connection.
...
[Associate a public IP address to a virtual machine](https://learn.microsoft.com/en-us/azure/virtual-network/ip-services/associate-public-ip-address-vm?tabs=azure-portal)    
...
![image info](./Q122_1.PNG)

Your company has the Azure environment shown in the network diagram exhibit.
As shown in the exhibit, an Azure firewall is deployed in SubnetA. All traffic within the network now traverses through SubnetB.
The IP addresses of the subnets are shown in the following table:

Name | IP address space

SubnetA : 192.168.1.0/20
SubnetB : 192.168.2.0/20
SubnetC : 192.168.3.0/20

Your manager wants SubnetB to give access to a site located outside Azure.
...
You should create an application rule to enable SubnetB to give access to a site located outside Azure. Application rules are used within an Azure firewall to define Fully Qualified Domain Names (FQDN), which can be accessed through specified subnets. As a result, hosts can access the site located outside Azure.
You should not create a private endpoint. A private endpoint is an interface that uses private IP addresses from virtual networks.
...
You should not add a Load Balancer rule. Azure Load Balancer enables you to load balance protocol flows on all ports. If you add a rule to the Load Balancer, you will be able to distribute incoming traffic, but will not allow subnets to give access to sites located outside Azure.
...
Your company has an Azure virtual network (VNet) that contains a subnet named SubnetA.
SubnetA contains a virtual machine that handles web application traffic. SubnetA has a network security group called NSG1.
You detect unexpected traffic in SubnetA.
You need to find details related to incoming IP traffic through NSG1.
...
You should use flow logs to find details related to incoming IP traffic through NSG1. The flow logs of a network security groups (NSG) map IP traffic through an NSG. Flow logs can be used for monitoring your network, validating isolation within your network, and detecting compromised IP addresses. Using this feature, you can know more about incoming and outgoing IP traffic passing by an NSG.
You should not use the Synapse Link. A Synapse Link is a service provided by Azure to monitor data analytics for operations within Azure Cosmos DB. This service cannot be used to find details related to incoming IP traffic through NSG1. Azure Cosmo DB is a NoSQL database used within mobile apps and web apps.
You should not use service tags. A service tag is a collection of IP address prefixes that can be used to simplify the creation of security rules.
You should not use Application Insights. This is an Azure Monitor feature that is used for monitoring the performance anomalies of live apps. This tool is used by DevOps experts. This solution will not enable you to find details related to incoming IP traffic through NSG1.
...
Your company has an Azure virtual network (VNet). You have two subnets within the VNet: SubnetA and SubnetB. SubnetA contains three Virtual Machines (VMs) named VM1, VM2, and VM3 and SubnetB contains a virtual machine named VM4.
VM1 and VM2 handle web application traffic.
VM3 handles the business logic.
VM4 hosts the database.

Your manager wants you to configure the traffic flow as follows:
1. Internet traffic reaches VM1 and VM2.
2. VM1 and Server2 communicate with VM3.
3. VM3 communicates with VM4.
...
You should use Application Security Groups (ASG) to meet the requirements. ASG is a feature that provides network security based on workloads rather than definite IP addresses. It also enables you to group VMs and filter the traffic inside them. ASG is used within network security groups (NSG) to define security rules for VMs and workloads.
You should not use IP flow. Internet Protocol (IP) flow is a feature that enables you to know when packets can access a VM or not, but cannot be used to direct internet traffic as required.
You should not use Azure Front Door. This solution is used to elevate the global routing of your web, but cannot be used to direct traffic. Azure Front Door is a global load-balancing solution that operates at layer 7 of the OSI model. It is beneficial to use Azure Front Door in case you want an additional security and filtering layer for your web application.
You should not use Azure Network Watcher. Azure Network Watcher is a service that enables you to monitor the health of your network and collect diagnostic logs.
...
[Introduction to Azure Network Watcher IP flow verify](https://learn.microsoft.com/en-us/azure/network-watcher/ip-flow-verify-overview)
...
Your company has been hosting its services in an Azure Landing Zone for the last six months. The Azure environment has recently had a third-party pen test carried out. The pen test has produced a report that shows a number of security issues that need to be addressed.
One of the highlighted issues in the report is that Azure Storage Accounts should be protected using virtual network service endpoints as they are currently configured for Public Access. When you go to configure the service endpoint on the first storage account, you receive a permissions error. You check your existing permissions and have Storage Blob Data Contributor role assigned to you.
You need to request the relevant role that will grant you permissions to be able to configure service endpoints on all storage accounts. Your request should adhere to the principle of least privilege.

What role should you request?
Choose the correct answer
...
You should request the Storage Account Contributor role that will allow you to configure service endpoints on all storage accounts. This role grants you the permission access to Microsoft.Network/virtual Networks/subnets/joinViaServiceEndpoint/action, which is required to make the relevant changes. This role meets the principle of least privilege as it only grants access to storage accounts.
You should not request the Contributor role. Although this is a high-privileged role that will and would give you the relevant access in order to configure service endpoints on all storage accounts in the tenant, it would also give you access to create and manage all other laas and PaaS services within the tenant. The Contributor role is the next role below the Owner role, therefore is a highly sensitive role. Requesting this role would not be adhering to the principle of least privilege.
You should not request the Storage File Data Privileged Contributor role. This is a specific storage account role that allows you to manage and modify files and directories within an Azure File share. It does not grant you permissions to configure service endpoints on a storage account, therefore it is not the correct role to request.
You should not request the Storage Blob Data Owner role. Although this role will provide you higher privileges than the existing Storage Blob Data Contributor role that is assigned to you, it is still not enough to allow you to configure service endpoints on all storage accounts. This role will grant you full access to the Azure Storage Blob containers within the environment, including being able to access data and control access to the containers.
...
[Configure Azure Storage firewalls and virtual networks](https://learn.microsoft.com/en-us/azure/storage/common/storage-network-security?toc=%2Fazure%2Fvirtual-network%2Ftoc.json&tabs=azure-portal#grant-access-from-a-virtual-network) 
